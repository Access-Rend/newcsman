...
namespace mpp_impl {
	                         ...
	                         fd_type *pstdin, fd_type *pstdout, fd_type *pstderr)
	{
		...
		idx.cb = sizeof(idx);
		idx.dwFlags |= STARTF_USESTDHANDLES;
		...
		idx.hStdInput = pstdin[PIPE_READ];
		idx.hStdOutput = pstdout[PIPE_WRITE];
		...
		if (startup.merge_outputs) {
			...
			idx.hStdError = pstdout[PIPE_WRITE];
		}
		else {
			// redirect stderr to a file
			idx.hStdError = pstderr[PIPE_WRITE];
		}
